...
$checks = [];
array_push($checks, ["name", lang::getText("missing_name")]);
array_push($checks, ["address", lang::getText("missing_address")]);
array_push($checks, ["postalNumber", lang::getText("missing_postalnumber")]);
array_push($checks, ["postalCity", lang::getText("missing_postalcity")]);
array_push($checks, ["email", false]);
array_push($checks, ["phone", false]);
$check = true;
$errorStr = "";
if(count($_POST) > 0) {
	foreach($checks as $v) {
...
if($check === false) {
	...
	shop::writeCartTabs("shipping", false);
} else {
	...
	shop::writeCartTabs("shipping", true);
?>
...
<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td><a href="#" onclick="obj('shippingForm').submit();">Gå till betalning</a></td></tr>
</table>
</form>
<?php
}